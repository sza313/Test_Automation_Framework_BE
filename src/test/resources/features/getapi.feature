Feature: Testing API GET
  ...
  Scenario: Testing the API GET endpoints
    ...
    Given Verify a single user by first name: Janet and last name: Weaver with request: '/api/users/2'
    Given Verify a single resource by name: fuchsia rose with request: '/api/unknown/2'
